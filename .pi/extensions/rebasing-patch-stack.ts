...
// Files that are always regenerated by upstream releases and should never be
// merged — we keep our working-tree version verbatim.
// IMPORTANT: do NOT include package.json files here. They are handled by the
// json-merge driver (committed in .gitattributes), which deep-merges objects
// so our scripts additions (e.g. "self-build") survive rebase. Adding
// package.json patterns here would override json-merge with keep-ours and
// silently discard any scripts we add to packages/opencode/package.json.
...
const ensureMergeDrivers = async (pi: ExtensionAPI, repoRoot: string) => {
  ...
  // Use `true` (no-op exit 0) rather than `cp %A %D`: git does not substitute
  // %D in merge driver commands, so `cp %A %D` creates a literal file named
  // "%D" in the working tree. With `true`, git falls back to using %A (ours)
  // as the merge result, which is exactly what keep-ours wants.
  await pi.exec("git", ["config", "--local", "merge.keep-ours.name", "Keep ours (version bump)"], { cwd: repoRoot })
  await pi.exec("git", ["config", "--local", "merge.keep-ours.driver", "true"], { cwd: repoRoot })
